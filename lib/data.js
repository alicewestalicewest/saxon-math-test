...
// q7 6 sub-keys share 5 pts total (0.84+0.83+0.84+0.83+0.83+0.83 = 5.00)
const POINTS = {
...
function buildEmailBody(sub) {
  ...
  if (q7ok) {
    ...
    const ax=graded.results.q7ax.answer||"?", ay=graded.results.q7ay.answer||"?";
    const bx=graded.results.q7bx.answer||"?", by=graded.results.q7by.answer||"?";
    const cx=graded.results.q7cx.answer||"?", cy=graded.results.q7cy.answer||"?";
    wrongList += `\n  Q7: A'(${ax},${ay})  B'(${bx},${by})  C'(${cx},${cy})\n`;
    wrongList += `       Correct: A'(-5,1)  B'(-1,1)  C'(-3,3)\n`;
    wrongList += `       Solution: ${SOLUTIONS.q7}\n`;
  }

  ALL_KEYS.filter(k => !Q7K.includes(k)).forEach(k => {
    const r = graded.results[k];
    if (r.correct) {
      correctList += `  ${k.toUpperCase()}  OK\n`;
    } else {
      wrongList += `\n  ${k.toUpperCase()}: Your answer: ${r.answer||"blank"}\n`;
      wrongList += `       Correct: ${(ANSWER_KEY[k]||[""])[0]}\n`;
      wrongList += `       Solution: ${SOLUTIONS[k]||""}\n`;
    }
  });
  ...
  Object.keys(FACTS_ANSWERS).forEach((fk, i) => {
    ...
    if (!fr) return;
    const mark = fr.correct ? "OK" : "X  (correct: " + fr.expected + ")";
